...
const REGION_PREFERENCE_ORDER = [
  "香港","日本","新加坡","韩国","台湾","美国","澳门",
  "俄罗斯","德国","英国","法国","荷兰","加拿大","澳大利亚","新西兰",
  "阿联酋","土耳其","印度","泰国","越南","菲律宾","马来","印尼",
  "关岛","北马里亚纳群岛","英属印度洋领地",
  "纽埃","帕劳","萨摩亚","美属萨摩亚","托克劳","图瓦卢","基里巴斯","瓦努阿图",
  "诺福克岛","密克罗尼西亚","法属波利尼西亚","瓦利斯和富图纳","圣皮埃尔和密克隆",
  "瑞士","瑞典","挪威","丹麦","芬兰","比利时","意大利","西班牙","葡萄牙",
  "波兰","捷克","奥地利","匈牙利","罗马尼亚","乌克兰","白俄罗斯","哈萨克斯坦",
  "巴基斯坦","沙特阿拉伯","卡塔尔","阿曼","巴林","科威特","伊朗","伊拉克",
  "蒙古","朝鲜","老挝","柬埔寨","缅甸","尼泊尔","不丹","孟加拉国","斯里兰卡",
  "南非","墨西哥","巴西","阿根廷","智利","秘鲁",
  "巴勒斯坦","科索沃","库克群岛","法属圭亚那","荷属加勒比","法属圣马丁",
  "荷属圣马丁","圣巴泰勒米","瓜德罗普","马提尼克","阿鲁巴","福克兰群岛",
  "特克斯和凯科斯群岛","马约特","南苏丹","几内亚比绍","圣多美和普林西比",
  "中国大陆"
];
...
const PROTOCOL_PREFERENCE_ORDER = [
  "AnyTLS","Hysteria2","TUIC","Hysteria","VLESS","Trojan","WireGuard",
  "Juicity","Naive","VMess","SS","SSR","SOCKS5","HTTP","HTTPS","Snell","Brook","SSH"
];
...
const valueArray = [
  "2×","3×","4×","5×","6×","7×","8×","9×","10×",
  "DL","IPLC","IEPL","Kern","Edge","Pro","Std","Spec","Exp","Biz","Game",
  "Buy","LB","CF","UDP","GPT"
];
...
function isDisabledCodeAlias(s) {
  return !!DISABLED_CODE_ALIASES[String(s || "").toUpperCase()];
}
...
function regionFlag(region) {
  return region.flag || flagFromCode(region.en);
}
...
function addCjkMatcher(alias, region, score) {
  ...
  if (!CJK_MATCHERS_BY_FIRST[first]) CJK_MATCHERS_BY_FIRST[first] = [];
  CJK_MATCHERS_BY_FIRST[first].push({
    alias: alias,
    region: region,
    score: score
  });
...
function buildRegionDbAndMatchers() {
  ...
  db.forEach(function (region) {
    ...
    region.aliases.forEach(function (alias) {
      ...
      const score = 10000 + a.length;

      if (/[\u4e00-\u9fff]/.test(a) || /[^\x00-\x7F]/.test(a)) {
        addCjkMatcher(a, region, score);
      } else {
        addAsciiMatcher(a, region, score);
      }
    });
  });

  Object.keys(CJK_MATCHERS_BY_FIRST).forEach(function (k) {
    CJK_MATCHERS_BY_FIRST[k].sort(function (a, b) {
      return b.score - a.score;
    });
  });

  ASCII_MATCHERS.sort(function (a, b) {
    return b.score - a.score;
  });
...
function matchCjkRegion(text) {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    const list = CJK_MATCHERS_BY_FIRST[ch];
    if (!list) continue;
